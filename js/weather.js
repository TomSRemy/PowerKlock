...
function ccOpenFullscreen() {
  ...
  (window.pkOpenOrUpdate || window.pkOpenFullscreen)({
    ...
    chartSource: {
      rebuildInto: (canvas) => {
        // ── HTML view (Heatmap) ───────────────────────────────────────────
        // CC Heatmap renders HTML into #cc-heatmap, not into the Chart.js
        // canvas. Mirror the DOM into the FS canvas container.
        if (view === 'heatmap') {
          const src = document.getElementById('cc-heatmap');
          if (src && canvas && canvas.parentNode) {
            canvas.style.display = 'none';
            let mirror = canvas.parentNode.querySelector('.cc-fs-html-mirror');
            if (!mirror) {
              mirror = document.createElement('div');
              mirror.className = 'cc-fs-html-mirror';
        ...
        // ── Chart.js views (Lines, Profile, Bands, Spread) ────────────────
        // Remove any leftover HTML mirror from a previous Heatmap view.
        if (canvas && canvas.parentNode) {
          const mirror = canvas.parentNode.querySelector('.cc-fs-html-mirror');
