...
async function loadPrices() {
  _demoCached = null; // clear demo cache so real data takes over
...
const FLAG_MAP = {
  ...
  IT_NORD:'🇮🇹', IT_SICI:'🇮🇹', IT_CALA:'🇮🇹', IT_BRNN:'🇮🇹', IT_PRGP:'🇮🇹',
  AT:'🇦🇹', CH:'🇨🇭', CZ:'🇨🇿', SK:'🇸🇰', HU:'🇭🇺', PL:'🇵🇱',
  RO:'🇷🇴', HR:'🇭🇷', SI:'🇸🇮', RS:'🇷🇸', BG:'🇧🇬', GR:'🇬🇷',
  MK:'🇲🇰', ME:'🇲🇪', AL:'🇦🇱', TR:'🇹🇷',
  DK_W:'🇩🇰', DK_E:'🇩🇰', SE:'🇸🇪', SE_1:'🇸🇪', SE_2:'🇸🇪', SE_3:'🇸🇪', SE_4:'🇸🇪',
...
function renderPricesTableBody() {
  ...
  const negFmt = (h) => {
    ...
    // h is in decimal hours (e.g. 0.75 = 45min = 3 slots)
    // Round to nearest quarter-hour, display as whole hours for DA
    const totalSlots = Math.round(h * 4); // number of 15-min slots
    const totalHours = Math.floor(totalSlots / 4);
    const remSlots   = totalSlots % 4;
    const label = totalHours > 0
      ? (remSlots > 0 ? `${totalHours}h${remSlots*15}` : `${totalHours}h`)
      : `${remSlots*15}min`;
  ...
  const rows = sorted.map((z, i) => {
    ...
    // Color based on daily average, not individual slots
