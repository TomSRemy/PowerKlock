...
function setRowBandWindow(idx, key) {
  ...
  // Update fullscreen pills (if open)
  const fsHost = document.getElementById(`fs-row-band-pills-${idx}`);
  if (fsHost) fsHost.innerHTML = _rowBandPillsHTML(idx);
...
// When a fullscreen overlay (pkOpenFullscreen) is open and tracks _fsChart,
// copy the inline chart's datasets so they stay in sync.
function _syncRowChartToFullscreen(srcChart) {
  const fs = document.getElementById('pk-fs-overlay');
  if (!fs || !fs._fsChart) return;
  try {
    fs._fsChart.data.datasets = JSON.parse(JSON.stringify(srcChart.data.datasets));
    fs._fsChart.update('none');
  } catch (e) {
    console.warn('[fs] sync to fullscreen chart failed', e);
  }
}
...
function openRowFullscreen(idx) {
  ...
  if (typeof window.pkOpenFullscreen !== 'function') {
    console.error('[openRowFullscreen] pkOpenFullscreen is not loaded');
    return;
  }
  ...
  // ── Clone KPIs from inline drill-down ──
  const inlineKpis = document.getElementById(`row-kpis-${idx}`);
  const kpisHtml = inlineKpis ? inlineKpis.outerHTML : '';

  // ── Clone breakdown table from inline <details> ──
  const inlineDetails = inner.querySelector('details');
  const tableEl = inlineDetails ? inlineDetails.querySelector('table') : null;
  const tableHtml = tableEl ? (
    `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
       <span style="font-size:10px;font-weight:700;letter-spacing:0.08em;color:var(--tx2);text-transform:uppercase">15-min breakdown</span>
       <span style="font-size:9px;color:var(--tx3);font-family:'JetBrains Mono',monospace">${tableEl.querySelectorAll('tbody tr').length} slots</span>
     </div>
     ${tableEl.outerHTML}`
  ) : null;

  // ── Clone Market Read banner (analysis) ──
...
function buildRowCSV(idx, code, dateStr) {
  const inner = document.getElementById(`row-detail-inner-${idx}`);
...
/* ════════════════════════════════════════════════════════════════════════════
   prices-hod-shape.js · PowerKlock
   ─────────────────────────────────────────────────────────────────────────────
   Day-Ahead Daily · Drill row · sub-tab "Hour-of-day shape"
   ...
   ── window.prHodRenderShape(idx, zone)  — main entry, mounts the sub-tab content
   ── window.prHodSetPeriod(p)            — sub-toggle Période setter
   ── window.prHodSetYears(y)             — sub-toggle Years setter
   ── window.prHodTogglePlay()            — toggle animation
   ── window.prHodHighlightYear(y)        — highlight one year in stack
   ════════════════════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  // PUBLIC API
  // ════════════════════════════════════════════════════════════════
  window.prHodRenderShape = function (idx, zone) {
    STATE.currentZone = zone;
    STATE.currentIdx  = idx;
    const host = document.getElementById(`row-hod-shape-${idx}`);
    if (!host) return;

    host.innerHTML = `
      <!-- Sub-toggle bar -->
      <div class="pk-filters-bar" id="row-hod-toolbar-${idx}" style="flex-wrap:wrap;gap:10px">
        <!-- Période -->
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <div id="row-hod-period-pills-${idx}" style="display:inline-flex;gap:4px"></div>
        </div>
        <!-- Years scope -->
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <div id="row-hod-years-pills-${idx}" style="display:inline-flex;gap:4px"></div>
        </div>
        ...
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <select id="row-hod-highlight-${idx}" onchange="prHodHighlightYear(this.value)" style="background:var(--bg);border:1px solid var(--bd);color:var(--tx);font-size:11px;padding:3px 8px;border-radius:4px;font-family:inherit;cursor:pointer;color-scheme:dark"></select>
        </div>
        <div class="pk-filters-bar-spacer"></div>
        <!-- Play button -->
        <button class="pk-btn-primary" id="row-hod-play-btn-${idx}" onclick="event.stopPropagation();prHodTogglePlay()" title="Cycle through years one-by-one">▶ Play</button>
        <button class="pk-btn-ghost" onclick="event.stopPropagation();(function(){var c=window._prHodChart;if(c&&c.resetZoom)c.resetZoom();})()" title="Reset zoom">↺ Reset</button>
      </div>

      <!-- Chart container -->
      <div style="position:relative;height:360px;margin-top:12px">
        <canvas id="row-hod-canvas-${idx}" style="width:100%;height:360px"></canvas>
        <!-- Big year label, bottom-left, visible when highlight or play active -->
        <div id="row-hod-year-label-${idx}" style="position:absolute;left:6px;bottom:6px;font-family:'Inter',sans-serif;font-size:32px;font-weight:800;letter-spacing:-.02em;color:var(--acc);opacity:0;pointer-events:none;transition:opacity .25s, color .25s">2026</div>
      </div>

      <!-- Market read banner anchor -->
      <div id="row-hod-banner-${idx}" style="margin-top:14px"></div>
      ...
      <details style="margin-top:14px" open>
        ...
        <div id="row-hod-breakdown-${idx}" style="margin-top:8px;overflow-x:auto"></div>
      </details>
    `;

    _hodPaintControls(idx);
    _hodRedraw(idx);
  };
  ...
  window.prHodSetYears = function (y) {
  ...
  window.prHodTogglePlay = function () {
    if (STATE.playing) {
      _hodStopPlay();
    } else {
      _hodStartPlay();
  ...
  // ════════════════════════════════════════════════════════════════
  function _hodPaintControls(idx) {
    ...
    // Période pills
    const periodHost = document.getElementById(`row-hod-period-pills-${idx}`);
    if (periodHost) {
      periodHost.innerHTML = PERIOD_OPTIONS.map(p =>
        pkPill({ label: p, active: p === STATE.period, onClick: `prHodSetPeriod('${p}')` })
      ).join('');
    }

    // Years pills
    const yearsHost = document.getElementById(`row-hod-years-pills-${idx}`);
    if (yearsHost) {
      yearsHost.innerHTML = YEARS_OPTIONS.map(y =>
        pkPill({ label: y, active: y === STATE.yearsScope, onClick: `prHodSetYears('${y}')` })
      ).join('');
    }

    // Year highlight dropdown
    const dd = document.getElementById(`row-hod-highlight-${idx}`);
    if (dd) {
  ...
  function _hodRedraw(idx) {
    ...
    if (window._prHodChart) {
      try { window._prHodChart.destroy(); } catch (_) {}
    }

    window._prHodChart = new Chart(canvas, {
      type: 'line',
      data: { labels, datasets },
      options: {
        responsive: true,
  ...
  function _hodRenderBanner(idx, years, shapes) {
    const host = document.getElementById(`row-hod-banner-${idx}`);
    if (!host) return;
  ...
  function _hodRenderBreakdown(idx, years, shapes) {
    const host = document.getElementById(`row-hod-breakdown-${idx}`);
    if (!host) return;
