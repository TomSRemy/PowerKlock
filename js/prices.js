...
function renderPricesTableBody() {
  ...
  const negFmt = (h) => {
    ...
    return `<span style="color:var(--warn);font-weight:600">${hours}h${String(mins).padStart(2,'0')}</span>`;
  };
  ...
  const rows = sorted.map((z, i) => {
    ...
    const negCount = h24spark.filter(v=>v<0).length;
    const sparkPositive = negCount === 0 ? true : negCount >= h24spark.length * 0.5 ? false : 'mixed';
    ...
    const html = `<tr style="cursor:pointer" onclick="togglePriceRow(${i}, event)" title="Expand 15-min chart">
      <td style="font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:700;color:var(--acc)">${z.flag||''} ${z.code}</td>
      <td style="font-size:11px;color:var(--tx2)">${meta.country}</td>
