...
return `<div style="background:var(--bg3);border:1px solid var(--bd);border-radius:6px;padding:8px 10px;text-align:center"> <div style="font-size:10px;color:var(--tx3);margin-bottom:4px">${m}</div> <div style="font-size:16px;font-weight:700;color:${col}">${h.toFixed(0)}<span style="font-size:10px;font-weight:400;color:var(--tx3)">h</span></div> </div>`;
}).join(’’);
}
...
return `<label style="display:flex;align-items:center;gap:8px;padding:4px 8px;border-radius:4px;cursor:pointer;font-size:11px;${on?'background:rgba(20,211,169,0.08)':''}"> <input type="checkbox" ${on?'checked':''} onchange="toggleGlobalZone('${code}')" style="cursor:pointer;accent-color:${color}"> <span style="font-family:'JetBrains Mono',monospace;color:var(--tx2);min-width:60px">${flag} ${code}</span> <span style="width:8px;height:8px;border-radius:50%;background:${color};margin-left:auto"></span> </label>`;
}).join(’’);
}
...
if (window._HO_CHART) {
try { window.*HO_CHART.destroy(); } catch (*) {}
window._HO_CHART = null;
}
window._HO_OPEN_ZONE = null;
}
...
return `<button onclick="setHistSingleTab('${t.id}')" style=" padding:5px 12px;border-radius:4px;font-size:11px;font-weight:600;cursor:pointer; border:none;background:${on?'var(--bg3)':'transparent'}; color:${on?'var(--text)':'var(--tx3)'}; letter-spacing:.03em; ">${t.label}</button>`;
}).join(’’);
}
...
return `<button onclick="setHistMultiTab('${t.id}')" style=" padding:5px 12px;border-radius:4px;font-size:11px;font-weight:600;cursor:pointer; border:none;background:${on?'var(--bg3)':'transparent'}; color:${on?'var(--text)':'var(--tx3)'}; letter-spacing:.03em; ">${t.label}</button>`;
}).join(’’);
}
...
return `<tr> <td style="font-family:'JetBrains Mono',monospace;font-weight:600">${monthLabel}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace">${fmt(r.avg)}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace">${fmt(r.peakAvg)}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace">${fmt(r.offAvg)}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace">${fmt(r.spread)}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace;color:${r.negH > 0 ? _HIST_WARN : 'var(--tx3)'}">${r.negH || 0}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace;color:var(--tx3)">${r.days}</td> <td style="text-align:right;font-family:'JetBrains Mono',monospace;color:${vsLYColor}">${vsLY}</td> </tr>`;
}).join(’’);
}
